...
public:
    ...
    ThoiGian(): hour(0), minute(0){}    //hàm tạo
    ThoiGian(int h, int m):hour(h), minute(m){} //hàm tạo 2 tham số

    ThoiGian operator++()   //hàm naphj chồng toán tử tiền tố
    {
        ++minute;
        if(minute>=60)
        {
    ...
    ThoiGian operator+(ThoiGian &Tg)    //hàm nạp chồng toán tử + 1 ngôi
    {
        ThoiGian T;
        T.hour = this->hour+Tg.hour;
        T.minute=this->minute+Tg.minute;
