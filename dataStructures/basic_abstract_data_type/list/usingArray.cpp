...
void Insert_List(ElementType X, Position P, List &L) // Chèn thêm phần tử X vào Danh sách L tại vị trí P
{
    if (L.Last == Maxlength)
...
Position Locate(ElementType X, List L)  //Tìm kiếm phần tử X trong danh sách
{
    Position P;
    int Found =0;
    P=First_List(L);
...
void Distinc(List &L) // Loại bỏ các phần tử trùng nhau trong danh sách
{
    for (int i = First_List(L) - 1; i < End_List(L) - 1; i++)
        for (int j = i + 1; j < End_List(L) - 1; j++)
            if (L.Elements[i] == L.Elements[j])
